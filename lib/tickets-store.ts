...
function orderToItem(order: TicketOrder) {
  return {
    ...
    ...(order.paymentId ? { paymentId: { S: order.paymentId } } : {}),
    ...(order.receiptNumber ? { receiptNumber: { S: order.receiptNumber } } : {}),
...
function itemToOrder(
  ...
  return {
    ...
    paymentId: item.paymentId?.S ?? undefined,
    receiptNumber: item.receiptNumber?.S ?? undefined,
